...
class GraphStore:
    ...
    async def store_section_graph(self, section: Dict, document_id: str) -> str:
        """
        ...
        logger.info(
            "Storing section graph {} for document {}", section_graph_id, document_id
        )
    ...
    async def get_section_graph(self, section_graph_id: str) -> Dict:
        ...
        async with driver.session(database=self.database) as session:
            try:
                ...
                logger.error(
                    "Failed to retrieve section graph {}: {}", section_graph_id, e
                )
                raise
    ...
    async def traverse_section_graph(
        ...
        """
        logger.info(
            "Traversing section graph {} for keywords: {}",
            section_graph_id,
            query_keywords,
        )
        ...
        async with driver.session(database=self.database) as session:
            try:
                ...
                keyword_filters = " OR ".join(
                    [
                        f"toLower(e.name) CONTAINS toLower($kw{i})"
                ...
                if not matched_entities:
                    logger.debug(
                        "No matching entities for keywords, falling back to raw_text"
                    )
                    section_result = await session.run(
                        """
                        MATCH (s:Section {id: $section_graph_id})
                ...
                for entity in matched_entities:
                    ...
                    context_parts.append(
                        f"Entity: {entity_name} (Type: {entity_label})"
                    )
                ...
                context = "\n".join(context_parts)
                logger.info(
                    "Generated context with {} matched entities", len(matched_entities)
                )
                return context

            except Exception as e:
                logger.error(
                    "Failed to traverse section graph {}: {}", section_graph_id, e
                )
                raise

    async def list_all_section_graphs(
    ...
    async def delete_document_graphs(self, document_id: str) -> int:
        ...
        async with driver.session(database=self.database) as session:
            try:
                ...
                logger.info(
                    "Deleted {} section graphs for document {}",
                    deleted_count,
                    document_id,
                )
    ...
    async def delete_section_graph(self, section_graph_id: str) -> bool:
        ...
        async with driver.session(database=self.database) as session:
            try:
                ...
                logger.info(
                    "Deleted section graph {}: {}", section_graph_id, deleted_count > 0
                )
                return deleted_count > 0

            except Exception as e:
                logger.error(
                    "Failed to delete section graph {}: {}", section_graph_id, e
                )
                raise
