...
def analyze_image(file_obj):
    ...
    caption, pred_dept, confidence, inference_time = itt.generate_caption_from_bytes(
        image_bytes
    )
    logger.debug(
        f"Inference | caption='{caption}' | dept='{pred_dept}' | confidence={confidence:.2f} | time={inference_time:.3f}s"
    )
...
def get_or_create_department(pred_dept):
    ...
    logger.debug(
        f"Falling back to default department | id=1 | name ={default_department.name}"
    )
