...
class ComplaintGroup(models.Model):
    ...
    department = models.ForeignKey(
        ...
        related_name="complaint_groups",
        null=True,
        blank=True,
    )  # nullable (temporarily) once i shld reset db.sqlite3
...
class Complaint(models.Model):
    ...
    citizen = models.ForeignKey(
        CitizenProfile, on_delete=models.CASCADE, related_name="complaints"
    )
    department = models.ForeignKey(
...
class Evidence(models.Model):
    ...
    complaint = models.ForeignKey(
        Complaint, on_delete=models.CASCADE, related_name="evidences"
    )
...
class ComplaintCount(models.Model):
    complaint = models.OneToOneField(
        Complaint, on_delete=models.CASCADE, related_name="count"
    )
    ...
    def __str__(self):
        return f"Complaint {self.complaint.id} - Closed {self.closed_count} times"
